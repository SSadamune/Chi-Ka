...
class TodofukenParser{
    static void parseTodofuken (String tdfk, int maxHousePages, int maxMansionPages, ICollector collector) throws IOException {
        ...
        for (int nc : houseCodes) {
            parseProperty(tdfk, nc, "house", collector);
            properties += 1;
        ...
        for (int nc : mansionCodes) {
            parseProperty(tdfk, nc, "mansion", collector);
            properties += 1;
    ...
    static void parseProperty(String todofuken, int ucCode, String propertyKind, ICollector collector) throws IOException {
        ...
        collector.collect(doc, url, propertyKind);
    }

    public static void main(String[] args) throws IOException {
        ICollector collector = new TableDataCollector();
        parseTodofuken("nerima", 1, 1, collector);
        collector.output();
    }
}
